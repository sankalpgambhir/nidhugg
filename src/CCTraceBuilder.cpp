...
void CCTraceBuilder::store(const SymData &sd){
  assert(false && "Cannot happen");
  abort();
}

void CCTraceBuilder::atomic_store(const SymData &sd){
...
void CCTraceBuilder::atomic_rmw(const SymData &sd){
  record_symbolic(SymEv::Rmw(sd));
  do_load(sd.get_ref());
  do_atomic_store(sd);
}

void CCTraceBuilder::load(const SymAddrSize &ml){
...
void CCTraceBuilder::compare_exchange
...
void CCTraceBuilder::full_memory_conflict(){
...
void CCTraceBuilder::join(int tgt_proc){
  record_symbolic(SymEv::Join(tgt_proc));
  curev().may_conflict = true;
  add_happens_after_thread(prefix_idx, tgt_proc);
}

void CCTraceBuilder::mutex_lock(const SymAddrSize &ml){
...
void CCTraceBuilder::mutex_lock_fail(const SymAddrSize &ml){
...
void CCTraceBuilder::mutex_trylock(const SymAddrSize &ml){
  Mutex &mutex = mutexes[ml.addr];
...
void CCTraceBuilder::mutex_unlock(const SymAddrSize &ml){
...
void CCTraceBuilder::mutex_init(const SymAddrSize &ml){
...
void CCTraceBuilder::mutex_destroy(const SymAddrSize &ml){
...
bool CCTraceBuilder::cond_init(const SymAddrSize &ml){
...
bool CCTraceBuilder::cond_signal(const SymAddrSize &ml){
  llvm::dbgs() << "condvars not supported\n";
  abort();
...
bool CCTraceBuilder::cond_broadcast(const SymAddrSize &ml){
  llvm::dbgs() << "condvars not supported\n";
  abort();
...
bool CCTraceBuilder::cond_wait(const SymAddrSize &cond_ml, const SymAddrSize &mutex_ml){
  llvm::dbgs() << "condvars not supported\n";
  abort();
  {
...
bool CCTraceBuilder::cond_awake(const SymAddrSize &cond_ml, const SymAddrSize &mutex_ml){
  llvm::dbgs() << "condvars not supported\n";
  abort();
...
int CCTraceBuilder::cond_destroy(const SymAddrSize &ml){
  llvm::dbgs() << "condvars not supported\n";
  abort();
...
void CCTraceBuilder::register_alternatives(int alt_count){
...
void CCTraceBuilder::compute_unfolding() {
...
void CCTraceBuilder::record_symbolic(SymEv event){
  if (!replay) {
    assert(!seen_effect);
    /* New event */
...
bool CCTraceBuilder::is_lock(unsigned i) const {
  return prefix[i].sym.kind == SymEv::M_LOCK;
}

bool CCTraceBuilder::does_lock(unsigned i) const {
  auto kind = prefix[i].sym.kind;
  return kind == SymEv::M_LOCK || kind == SymEv::M_TRYLOCK;
}

bool CCTraceBuilder::is_trylock_fail(unsigned i) const {
  return prefix[i].sym.kind == SymEv::M_TRYLOCK_FAIL;
}

bool CCTraceBuilder::is_unlock(unsigned i) const {
...
bool CCTraceBuilder::is_lock_type(unsigned i) const {
...
bool CCTraceBuilder::is_cmpxhgfail(unsigned i) const {
...
bool CCTraceBuilder::can_rf_by_vclocks
...
bool CCTraceBuilder::can_swap_by_vclocks(int r, int w) const {
  if (happens_before(prefix[r], prefix[w].above_clock)) return false;
  return true;
}

bool CCTraceBuilder::can_swap_lock_by_vclocks(int f, int u, int s) const {
...
std::vector<bool> CCTraceBuilder::causal_past(int decision) const {
...
void CCTraceBuilder::causal_past_1(std::vector<bool> &acc, unsigned i) const{
...
inline Option<unsigned> CCTraceBuilder::
